...
StartFeetButton.onclick = () =>{
    ...
    if (feet===null) startFeet=0;
    else startFeet = parseInt(feet);
    UpdateStartText();
}
StartInchesButton.onclick = () =>{
    ResetResults();
    const inches = prompt("Enter start inches:");
    if (inches===null) startInches=0;
    else startInches = parseInt(inches);
    if(startInches>=12) {
...
incrementButton.onclick = () =>{
    ...
    if (inches===null) increment=0;
    else increment = parseInt(inches);
